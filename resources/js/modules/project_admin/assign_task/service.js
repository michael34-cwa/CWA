  ...
  return dispatch => (  
    new Promise((resolve, reject) => {
     // dispatch(AssignTaskActions.spinerAdd(transformResponse()));
      Http.post(`course_assign/task/${id}`, transformRequest(params))
        .then(res => {
...
export function AssignTaskListRequest({ id, pageNumber = 1, value = '', url = "/course_assign/task" }) {  
         return dispatch => {
           dispatch(AssignTaskActions.spinerAdd(transformResponse()));
           url = url + '/' + id;
...
export function CourseTaskListRequest({ id,url = "/course_assign/task_by_id" }) {
  return dispatch => {
